...
function ProductLiked() {
    ...
    const [isLoggedIn, setIsLoggedIn] = useState<boolean>(false); // Để kiểm tra trạng thái đăng nhập

    const getLikedProducts = async (category: string | null = null, query: string = "") => {
        try {
            setLoading(true);
            setError("");
            setSuccess("");
            ...
            if (token) {
                ...
                const { data } = await axios.get("/users/liked-products", {
                    headers: {
                ...
                setProducts([]);
                setNoResults(true); // Hiển thị thông báo không có sản phẩm
            }
        } catch (error) {
    ...
    useEffect(() => {
        getLikedProducts(selectedCategory, searchQuery);
    }, [selectedCategory, searchQuery]);

    const handleCategorySelect = (category: string | null) => {
        setSelectedCategory(category);
        setCurrentPage(1);
        getLikedProducts(category, searchQuery);
    };

    const handleSearch = (query: string) => {
        setSearchQuery(query);
        setCurrentPage(1);
        getLikedProducts(selectedCategory, query);
    };
                ...
                >
                    {noResults ? (
                        isLoggedIn ? (
                            ...
                            <Typography variant="h3">Hãy đăng nhập để thích sản phẩm</Typography>
                        )
                    ) : (
